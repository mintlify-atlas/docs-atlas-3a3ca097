...
<Steps>
  ...
  <Step title="Run cells in order">
    Execute every cell from top to bottom. The notebook is divided into seven sequential steps:
    ...
    | **6 — Assemble GAN** | Calls `build_gan(generator, discriminator)`, freezes the discriminator weights (`discriminator.trainable = False`), and compiles the combined model with `adam` optimizer and `binary_crossentropy` loss. |
    | **7 — Train, generate, classify, evaluate** | Runs the full GAN training loop, generates synthetic data, trains all six classifiers, and prints evaluation reports. |
  </Step>
</Steps>
...
After training, `generate_synthetic_data(generator, 492)` draws 492 random noise vectors from a standard normal distribution, passes them through the trained generator, and returns 492 synthetic fraud feature vectors. These are appended to the real dataset before classifiers are trained, producing a balanced training split.
